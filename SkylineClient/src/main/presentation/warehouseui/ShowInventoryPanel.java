...
public class ShowInventoryPanel {
	...
	/*	String[] t1 = new String[]{title1,title3};
		...
		scrollPane2.setBounds(tab.getX(), tab.getY()+panel.getHeight()/24,tab.getWidth(),table1.getRowHeight());
	...
	/*	String[] ot = new String[]{"区号","排号","架号","位号"};
		String[][] tl = new String[][]{{"A","1","2","1"},{"A","1","3","1"},{"A","1","4","1"},
				{"B","2","2","1"},{"K","1","2","1"},{"F","1","2","1"},{"A","1","9","1"},
				{"A","9","3","1"},{"D","1","2","1"},{"A","1","2","1"},{"C","1","2","1"},
				{"H","11","2","7"},{"D","1","2","1"},{"B","2","3","1"},{"C","2","1","1"}};
		*/
		String[] ot = new String[]{"","区号","排号","架号","位号"};
		...
		table1.getTableHeader().setPreferredSize(new Dimension(tab.getWidth(), panel.getWidth()/20));//设置表头高度
		table1.getTableHeader().setReorderingAllowed(false);//表头不可移动
		table1.setDragEnabled(false);
		table1.setVisible(true);
		scrollPane = new JScrollPane(table1);
		if(tl.length>=10){
			scrollPane.setBounds(tab.getX(), tab.getY()+panel.getHeight()/24+table1.getRowHeight(), tab.getWidth(), 9*table1.getRowHeight());
		}else{
			scrollPane.setBounds(tab.getX(), tab.getY()+panel.getHeight()/24+table1.getRowHeight(),tab.getWidth(), (table1.getRowCount()+1)*table1.getRowHeight());
		}
...
/*		outNum = new JLabel("出库数量");
		outNum.setFont(font);
		outNum.setBounds(panel.getWidth()*14/20,panel.getHeight()*49/60, panel.getWidth()*8/11, panel.getHeight()/15);
	...
	*/	
	...
	/*	outOfNum = new JTextField();
		outOfNum.setBounds(panel.getWidth()*14/20,panel.getHeight()*49/60, panel.getWidth()*8/11, panel.getHeight()/15);
		outOfNum.setVisible(true);
	*/
		all = new JTextField();
		all.setBounds(panel.getWidth()*34/40,panel.getHeight()*89
				/100, panel.getWidth()/18, panel.getHeight()/20);
		all.setVisible(true);
